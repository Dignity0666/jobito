      ...
      <div className={styles.mainGrid}>
        ...
        <div className={styles.chartCard}>
          ...
          <div className={styles.chartArea}>
            {chartData.length >= 2 ? (
              <svg className={styles.chartSvg} viewBox={`0 0 ${chartWidth} ${chartHeight + 10}`} preserveAspectRatio="none">
                <defs>
                ...
                {[0.25, 0.5, 0.75].map(pct => (
                  <line key={pct} x1={chartPadding} y1={chartPadding + pct * (chartHeight - chartPadding * 2)} x2={chartWidth - chartPadding} y2={chartPadding + pct * (chartHeight - chartPadding * 2)} className={styles.chartGrid} />
                ))}
                {/* Fill area */}
                ...
                {/* Dots */}
                {dots.map((dot, i) => (
                  <circle key={i} cx={dot.x} cy={dot.y} r={3} className={styles.chartDot} />
                ))}
              </svg>
            ) : (
          ...
          {isLoading ? (
            ...
            filteredActivities.map((log) => (
              <div key={log.logId} className={styles.tableRow}>
                ...
                {/* Time */}
                <span className={styles.cellTime} title={new Date(log.createdAt).toLocaleString()}>
