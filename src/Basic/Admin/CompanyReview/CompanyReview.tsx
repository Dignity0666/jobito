    ...
    <div className={styles.modalOverlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className={styles.modal}>
        ...
        <div className={styles.modalHeader}>
          <div>
            <div style={{ fontSize: '20px', fontWeight: 800, color: 'var(--color-text)', marginBottom: '4px' }}>
              {t("Criminal Record Review", "مراجعة السجل الجنائي")}
            </div>
        ...
        <div style={{ padding: '24px' }}>
          <div className={styles.modalDetailsGrid}>
            ...
            <DetailItem icon="👷" label={t("Classification", "التصنيف")} value={user.classification || '—'} />
          </div>

          {/* Document Previews */}
          <div style={{ marginTop: '20px' }}>
            <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--color-text-muted)', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              {t("Criminal Record Document", "وثيقة السجل الجنائي")}
            </div>
            <div style={{
              border: '2px dashed var(--color-border)',
              borderRadius: '16px',
  ...
  const fetchCompanies = async () => {
    ...
    try {
      ...
      if (pendingRes.ok) {
        ...
        pendingCompanies = (data.data || []).map((c: any) => ({ ...c, status: c.status || 'PENDING' }));
      }
      ...
      if (allRes.ok) {
        ...
        allCompanies = (data.data || []).map((c: any) => ({ ...c, status: c.status || 'PENDING' }));
      }
      ...
      <div className={styles.body}>
        ...
        <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          <h1 style={{ fontSize: '24px', fontWeight: 800, margin: 0, color: 'var(--color-text)' }}>
            {viewMode === "COMPANIES" ? t("Company Registration Requests") : t("Criminal Record Reviews", "مراجعة السجلات الجنائية")}
          </h1>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid var(--color-border-light)', background: 'var(--color-bg-tertiary)' }}>
                  <th style={{ padding: '16px 24px', fontSize: '12px', color: 'var(--color-text-muted)', textTransform: 'uppercase', textAlign: 'left', fontWeight: 700, letterSpacing: '0.5px' }}>
                    {viewMode === "COMPANIES" ? t("Company") : t("Applicant", "المتقدم")}
                  </th>
                  ...
                  <th style={{ padding: '16px 24px', fontSize: '12px', color: 'var(--color-text-muted)', textTransform: 'uppercase', textAlign: 'left', fontWeight: 700, letterSpacing: '0.5px' }}>{t("Submission Date", "تاريخ التقديم")}</th>
