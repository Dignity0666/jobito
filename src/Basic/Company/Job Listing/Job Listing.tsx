...
export default function JobListing() {
  ...
  console.log("Logged in as:", user?.name); // Use variable to fix lint
  ...
  const fetchJobs = useCallback(async () => {
    try {
      let companyId; 
      try {
        const profileRes = await apiFetch(`${API_BASE_URL}/companies/my/profile`);
        if (profileRes.ok) {
      ...
      if (!jobsRes.ok) {
        ...
        throw new Error(errData.message || `Failed to fetch jobs (Status: ${jobsRes.status})`);
      }
  ...
  const handleDelete = async (e: React.MouseEvent, job: Job) => {
    ...
    if (!id || !window.confirm("هل أنت متأكد من حذف هذه الوظيفة؟")) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.message || "فشل حذف الوظيفة");
      }
    } catch {
  ...
  const handleToggleStatus = async (e: React.MouseEvent, job: Job) => {
    e.stopPropagation();
    const id = job.jobId || job.job_id;
    const currentActive = job.isActive !== undefined ? job.isActive : job.is_active;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setJobs(
          jobs.map((j) =>
            (j.jobId || j.job_id) === id ? { ...j, isActive: updated.isActive, is_active: updated.isActive } : j,
          ),
        );
      } else {
        const data = await res.json();
  ...
  if (loading) return <div className="jl-loading">جاري التحميل...</div>;
  if (error) return <div className="jl-error">خطأ: {error}</div>;
    ...
    <div className="job-listing-page">
      <div className="jl-header-area">
        <div className="jl-title-row">
          <div className="jl-title-group">
            <h1 className="jl-title">قائمة الوظائف</h1>
            <p className="jl-subtitle">
              إدارة الوظائف التي قمت بنشرها ومتابعة المتقدمين.
            </p>
          </div>
        </div>
      ...
      <div className="jl-main-card">
        <div className="jl-card-toolbar">
          <h2 className="jl-card-title">قائمة الوظائف ({jobs.length})</h2>
            ...
            <tbody>
              {jobs.map((job) => (
                <tr
                  ...
                  onClick={() =>
                    navigate("/Job details", { state: { jobId: job.jobId || job.job_id } })
                  }
                >
                  ...
                  <td className="jl-col-appl">
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/AllApplicants?jobId=${job.jobId || job.job_id}`);
                      }}
                    >
                      {job.appliedCount || 0}
                    </button>
